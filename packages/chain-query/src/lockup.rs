...
use cosmos_types::lockup::{
    LockedDenomRequest, LockedDenomResponse,
};

pub fn query_total_lock_up(deps: DepsMut, denom: String, duration: core::time::Duration) -> StdResult<u128> {
    let req = LockedDenomRequest{
        denom: denom,
        duration: duration
    ...
    let res_proto : proto::osmosis::lockup::LockedDenomResponse;
    res_proto = prost::Message::decode(&*res_x).unwrap();
    let res: LockedDenomResponse = TryFrom::try_from(res_proto).unwrap();

    Ok(res.amount.parse().unwrap())
}
